    ...
    pub async fn exec(self) -> Result<Profile, FerriormError> {
        ...
        macro_rules! build_update {
            ($qb_type:ty) => {
                ...
                first_set { return Err(FerriormError::Query("No fields to update"
                .into())); } qb.push(" WHERE 1=1"); self.r#where.build_where(& mut qb);
    ...
    pub async fn exec(self) -> Result<u64, FerriormError> {
        ...
        macro_rules! build_update_many {
            ($qb_type:ty) => {
                ...
                self.data.user_id { if ! first_set { qb.push(", "); } first_set = false;
                qb.push(concat!("\"", "user_id", "\" = ")); qb.push_bind(v); } if
                first_set { return Ok(0); } qb.push(" WHERE 1=1"); self.r#where
                .build_where(& mut qb); qb }
            };
        }
        match client {
            DatabaseClient::Postgres(_) => {
    ...
    pub async fn exec(self) -> Result<Profile, FerriormError> {
        ...
        macro_rules! build_upsert {
            ($qb_type:ty) => {
                ...
                .push(", "); } qb.push("\""); qb.push(* col); qb.push("\""); } qb
                .push(") VALUES ("); { let mut sep = qb.separated(", "); sep
                .push_bind(self.create.bio); sep.push_bind(self.create.avatar); sep
                .push_bind(self.create.user_id); let val = self.create.id
                .unwrap_or_else(|| uuid::Uuid::new_v4().to_string()); sep.push_bind(val);
                } qb.push(")"); qb.push(" ON CONFLICT (\"id\") DO UPDATE SET "); let mut
                first_set = true; if let Some(SetValue::Set(v)) = self.update.bio { if !
                first_set { qb.push(", "); } first_set = false; qb.push(concat!("\"",
                "bio", "\" = ")); qb.push_bind(v); } if let Some(SetValue::Set(v)) = self
                .update.avatar { if ! first_set { qb.push(", "); } first_set = false; qb
                ...
                "\" = ")); qb.push_bind(v); } if first_set { qb.push("\"id\" = \"id\"");
                } qb.push(" RETURNING *"); qb }
            };
        }
        match client {
