...
- **ferriorm-core**: Pure domain types (AST, Schema IR, ScalarType, DatabaseProvider). ZERO external dependencies (optional `serde` behind feature flag). Foundation crate -- all others depend on it.
- **ferriorm-parser**: PEG grammar (pest) to parse `.ferriorm` schema files into raw AST (`ast::SchemaFile`), then validates and resolves into Schema IR (`schema::Schema`). Entry point: `parse_and_validate()`.
- **ferriorm-codegen**: Generates type-safe Rust source files from Schema IR using `quote` + `prettyplease`. Produces model structs, filter/data/order submodules, relation types, CRUD query builders, and the `FerriormClient` entry point. Entry point: `generator::generate()`.
- **ferriorm-runtime**: Ships with user's app. Database client (`DatabaseClient` enum wrapping sqlx pools), filter types (`StringFilter`, `IntFilter`, etc.), parameterized `SqlBuilder`, ordering, transactions, and the `prelude` module.
...
- The `ferriorm-core` crate has a `serde` feature flag for JSON snapshot serialization (used by ferriorm-migrate)
